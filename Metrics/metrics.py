...
class AdaptiveECELoss(nn.Module):
    ...
    '''
    def __init__(self, n_bins=15, return_stats=False):
    ...
    def forward(self, softmaxes, labels):
        ...
        stats_dict = {'indices': [], 'individual_probs': [], 'individual_labels': [],
         'bins': [float(bin_) for bin_ in bin_boundaries.tolist()],
          'ce': [], 'ece': [], 'avg_conf': [], 'avg_acc': []}
        ...
        for bin_lower, bin_upper in zip(self.bin_lowers, self.bin_uppers):
            ...
            if prop_in_bin.item() > 0:
                accuracy_in_bin = accuracies[in_bin].float().mean()
                avg_confidence_in_bin = confidences[in_bin].mean()
                current_ece = torch.abs(avg_confidence_in_bin - accuracy_in_bin) * prop_in_bin
                ece += current_ece
                stats_dict['ce'].append(log_loss_criterion(torch.log(softmaxes[in_bin]), labels[in_bin].long()).item())
                stats_dict['ece'].append(current_ece.item())
                stats_dict['avg_conf'].append(avg_confidence_in_bin.item())
                stats_dict['avg_acc'].append(accuracy_in_bin.item())

            else:
                stats_dict['ce'].append(0)
                stats_dict['ece'].append(0)

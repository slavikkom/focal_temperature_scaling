#!/bin/bash
# Save OrganSMNIST train/val/test logits without running post-hoc calibration.
# Submit with:
# sbatch --array=0-<total_jobs_minus_1> organsmnist_save_logits_array.sh

#SBATCH --job-name=logits_organs
#SBATCH --output=slurm_logs_organsmnist/logits_job_%A_%a.out

#!/bin/bash
# Save CIFAR-100 train/val/test logits without running post-hoc calibration.
# Submit with:
# sbatch --array=0-<total_jobs_minus_1> cifar100_save_logits_array.sh

#SBATCH --job-name=logits_cf100
#SBATCH --output=slurm_logs_cifar100/logits_job_%A_%a.out
